 ...
 if(isset($_POST['change'])){
    ...
    if($result->num_rows > 0){
        ...
        if($conn->query($updateGroup) === TRUE){
            echo "Group assigned successfully! <br> <a href='assign.php'>Return to the group seting page! </a></br>";
        } else {
            echo "Error Unable to change student group! <br> <a href='assign.php'>Return to the group setting page! </a></br>";
        }
    } else {
...
if (isset($_POST['deleteNoti'])) {
    ...
    // Set the noti column for the specified user to an empty string
    $clearNotiQuery = "UPDATE users SET noti = '' WHERE studentid = '$studentid'";
    mysqli_query($conn, $clearNotiQuery);
